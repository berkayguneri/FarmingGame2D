...
public class Crop : MonoBehaviour
{
    ...
    {
        //is there a harvested animation --- hasat edilmiþ bir animasyon var mý
        if (cropDetails.isHarvestedAnimation && animator != null)
        {
            //if harvest sprite then add to sprite renderer --- hasat sprite ise sprite oluþturucuya ekle
        ...
        //hasat edilen bir animasyon var mý - Animasyon tamamlandýktan sonra bu mahsul oyun nesnesini yok edin
    ...
    private void SpawnHarvestedItems(CropDetails cropDetails)
    {
        for (int i = 0; i < cropDetails.cropProducedItemCode.Length; i++)
        {
            ...
            for (int j = 0; j < cropsToProduce; j++)
            {
                ...
                if (cropDetails.spawnCropProductAtPlayerPosition)
                {
