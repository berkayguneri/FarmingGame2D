...
public class VFXManager : SingletonMonobehaviour<VFXManager>
{
    ...
    {
        EventHandler.HarvetActionEffectEvent -= displayHarvestActionEvent;
    }
    ...
    private void displayHarvestActionEvent(Vector3 effectPosition, HarvestActionEffect harvestActionEffect)
    {
        switch (harvestActionEffect)
        {
            ...
            case HarvestActionEffect.deciduousLeavesFalling:
                GameObject deciduousLeavesFalling = PoolManager.Instance.ReuseObject(decidousLeavesFallingPrefab, effectPosition, Quaternion.identity);
